/* eslint-disable */
// https://github.com/archesproject/arches-vue-components/blob/dev/2.0.x/webpack/webpack-utils/patch-vue-compiler-sfc-type-resolution.js
const fs = require("fs");
...
function requireVueLoaderWithTypeResolutionPatch() {
    ...
    const frontendConfigurationDirectory = Path.join(
        __dirname,
        "..",
        "..",
    ...
    const {
        compilerOptions: { paths: tsconfigPathsRelativeToFrontendConfiguration },
    } = JSON.parse(
        fs.readFileSync(
            Path.join(frontendConfigurationDirectory, "tsconfig-paths.json"),
            "utf-8",
        ),
    );

    const archesApplicationPathAliases = Object.fromEntries(
        Object.entries(tsconfigPathsRelativeToFrontendConfiguration).map(
            ([alias, relativePaths]) => [
                alias,
                relativePaths.map((relativePath) =>
                    Path.resolve(frontendConfigurationDirectory, relativePath),
                ),
            ],
        ),
    );
    ...
    vueSingleFileComponentCompiler.compileScript = function (
        descriptor,
        options,
    ) {
        const virtualFileSystem = {
            ...
            readFile: (filePath, encoding) =>
                Path.basename(filePath) === "tsconfig.json"
                    ? virtualTsconfigContent
                    : ts.sys.readFile(filePath, encoding),
        };

        return originalCompileScript(descriptor, {
            ...options,
