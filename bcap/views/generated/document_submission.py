"""Document Submission API, scoped to the requesting user's own resources: a
...
class DocumentSubmissionViewMixin(ArchesResourceViewMixin):
    serializer_class = DocumentSubmissionSerializer


@extend_schema(tags=["External: document_submission"])
class DocumentSubmissionListView(
    DocumentSubmissionViewMixin,
    UserOwnedResourceMixin,
    ArchesModelAPIMixin,
    ListAPIView,
):
    """Collection endpoint for the requesting user's Document Submission resources.
...
@extend_schema(tags=["External: document_submission"])
class DocumentSubmissionView(
    DocumentSubmissionViewMixin,
    UserOwnedResourceMixin,
    ArchesModelAPIMixin,
    RetrieveAPIView,
):
    """Detail endpoint for a single Document Submission resource and its nested tiles.
