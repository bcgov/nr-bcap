...
THREAD_LINK = A.RELATED_SOURCE_MESSAGE
...
class MessageService:
    ...
    def detail_query(cls, message_id, user):
        ...
        return ThreadService.base_query(user, resource_ids=[str(message_id)])
    ...
    def prepare_create_payload(self, data, user):
        ...
        for alias in (*G.RESOLUTION_ALIASES[AUTHOR], *G.RESOLUTION_ALIASES[RECIPIENT]):
            set_payload_node(data, CONTENT, alias, None)
        ...
        PermitAccess.require_change(
            user, payload_resource_id(data, CONTENT, A.RESOURCE_CONTEXT)
        )
        return author_id

    @staticmethod
    def stamp_author(data, viewer):
    ...
    @staticmethod
    def inherit_thread_fields(data, viewer):
        ...
        thread_id = payload_resource_id(data, THREAD_LINK, THREAD_LINK)
        if not thread_id:
            return False
        thread_id = G.thread_id(thread_id)
        if not ThreadService.base_query(
            viewer.user, resource_ids=[thread_id], aliases=[A.RESOURCE_CONTEXT]
        ).exists():
            raise PermissionDenied("You cannot reply to this thread.")
        set_payload_node(
            data, THREAD_LINK, THREAD_LINK, resource_instance_value(thread_id)
        )
        ...
        if to:
            set_payload_node(data, CONTENT, A.RECIPIENT, resource_instance_value(to))
        set_payload_node(
            data, CONTENT, A.IS_INTERNAL, bool(root.get(G.node(A.IS_INTERNAL)))
        )
        ...
        for alias in (A.MESSAGE_TYPE, A.MESSAGE_SUBJECT):
            ...
            if inherited:
                set_payload_node(data, CONTENT, alias, inherited)
        return True

    @staticmethod
    def _is_new_internal_thread(data, viewer):
        """Whether a new thread is internal: only staff writing to staff or the
        Branch."""
        recipient_id = payload_resource_id(data, CONTENT, A.RECIPIENT)
        return bool(
