...
class ProcessRequirementService:
    ...
    def module_requirement_ids(self, permit_id: str):
        """Each process_module tile mapped to its requirement resource ids, which
        is what each requirement's messages file against."""
        ...
        for child in TileModel.objects.filter(
            ...
            requirements[str(child.parenttile_id)] |= referenced_resource_ids(
                [child], node
            )
