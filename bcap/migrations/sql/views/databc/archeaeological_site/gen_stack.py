...
def card(n):   return BY[n][3]
def ngid(n):   return BY[n][1]
def fields(n): return BY[n][4]
...
def obj_expr(n, indent):
    ...
    for f, nid, dt, fmt in fields(n):
        if dt == 'geojson-feature-collection':
            parts.append(f"'{f}', CASE WHEN g.geom IS NULL THEN NULL "
...
def geom_mv(name, nid, where_ng):
    ...
    return (f"DROP MATERIALIZED VIEW IF EXISTS {S.SCHEMA}.mv_geom_{name} CASCADE;\n"
...
def final_mv():
    ...
    for i, n in enumerate(TOPS):
        ...
        joins.append(f"LEFT JOIN {S.SCHEMA}.mv_{n} {a} ON {a}.resourceinstanceid = r.resourceinstanceid")
    for g in GEOM_MVS:
        cols.append(f"    g_{g}.{g}_geom,")
        joins.append(f"LEFT JOIN {S.SCHEMA}.mv_geom_{g} g_{g} ON g_{g}.resourceinstanceid = r.resourceinstanceid")
    jb = ",\n".join([f"        'resourceinstanceid', r.resourceinstanceid"] + jparts)
    return (f"DROP MATERIALIZED VIEW IF EXISTS {S.SCHEMA}.mv_resource_v1 CASCADE;\n"
            ...
            + "".join(f"CREATE INDEX mv_resource_v1_{g}_gix ON {S.SCHEMA}.mv_resource_v1 USING GIST ({g}_geom);\n"
                      for g in GEOM_MVS))
...
geoms_sql = []
geoms_sql.append(geom_mv('site_boundary', 'b18223c2-13ef-11f0-8695-0242ac170007',
                         'b18223c2-13ef-11f0-8695-0242ac170007'))
geoms_sql.append(geom_mv('unprotected_areas', '7c8eb1f8-44e2-4239-afaa-9cbf1fadf160',
                         '7c8eb1f8-44e2-4239-afaa-9cbf1fadf160'))

refresh_list = ([f"{S.SCHEMA}.mv_geom_{g}" for g in GEOM_MVS]
