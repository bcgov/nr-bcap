-- =====================================================================
--  archaeological_site :: PREFLIGHT.  Run ALL of this before building.
--  Ordered by blast radius. A, G and H can change the DDL.
-- =====================================================================


-- =====================================================================
-- A. CARDINALITY.  THE LOAD-BEARING ASSUMPTION.
-- Every array-vs-object decision came from reading your tree JSON. That is an
-- inference from ONE sample resource. Confirm it. Expected, from the tree:
...
--
-- ANY disagreement -> fix the spec and regenerate. Do not build and see.
...
-- =====================================================================
-- G. THE THREE NEW DATATYPES.  These do not appear in site_visit.
-- =====================================================================

-- G1. `url` (external_url). ASSUMED: {"url": "...", "url_label": "..."} with
--     url_label possibly an i18n object. url_obj() depends on this being right.
SELECT jsonb_pretty(tiledata -> '3ee73f28-ca40-11ed-af48-5254004d77d3') AS url_shape
FROM public.tiles
WHERE nodegroupid = '3ee73f28-ca40-11ed-af48-5254004d77d3'::uuid
  AND tiledata -> '3ee73f28-ca40-11ed-af48-5254004d77d3' IS NOT NULL
LIMIT 3;

-- G2. `borden-number-datatype` (custom). The generated view reads it with a bare
--     ->> and NO ::jsonb cast, so I am treating it as PLAIN TEXT, not i18n.
--     Expect a bare JSON string here, NOT {"en": {...}}.
SELECT jsonb_typeof(tiledata -> '7e15332c-1c54-11f0-b5bf-0242ac170007') AS borden_json_type,
       tiledata ->> '7e15332c-1c54-11f0-b5bf-0242ac170007'               AS borden_value
FROM public.tiles
WHERE nodegroupid = '034d1c32-13f2-11f0-9ff8-0242ac170007'::uuid
  AND tiledata -> '7e15332c-1c54-11f0-b5bf-0242ac170007' IS NOT NULL
LIMIT 3;
-- borden_json_type = 'string'  -> my reading is right.
-- borden_json_type = 'object'  -> it IS i18n; change the spec dt to 'string'.

-- G3. `non-localized-string` (reference_number, pid, pin). Same question.
SELECT jsonb_typeof(tiledata -> 'f5c343f3-217f-4ff0-a414-5dcaff74d2fa') AS pid_json_type,
       tiledata ->> 'f5c343f3-217f-4ff0-a414-5dcaff74d2fa'               AS pid_value
FROM public.tiles
WHERE nodegroupid = '1b622ab6-0d0f-11ed-98c2-5254008afee6'::uuid
  AND tiledata -> 'f5c343f3-217f-4ff0-a414-5dcaff74d2fa' is not null
LIMIT 3;


-- =====================================================================
-- H. EQUIVALENCE: does dropping the edit_log join change the row set?
-- Both edit_log joins in the generated views are LEFT JOINs, so the anti-join
-- only collapses fan-out - it never removes a tile. If that holds, every line
-- below reads ok. If via_view < via_tiles ANYWHERE, STOP: the reasoning is wrong
-- for your data and this stack must not be used.
-- =====================================================================
...
-- =====================================================================
-- I. NODE UUID SANITY. Every node uuid in the spec was transcribed by hand from
-- your view DDL. A mistyped uuid does NOT error - tiledata -> '<wrong>' just
-- returns NULL and you ship a permanently-empty column. Expect node_exists=true
-- on every row.
-- =====================================================================
...
-- =====================================================================
-- C. GEOMETRY.  Two geojson nodes. Confirm SRID, type mix and validity.
-- The stack uses ST_CollectionHomogenize(ST_Collect(...)) so homogeneous parts
-- collapse to MULTIPOLYGON. Mixed types will still yield GEOMETRYCOLLECTION -
-- this tells you whether that will happen.
-- =====================================================================
SELECT nodeid,
...
WHERE nodeid IN ('b18223c2-13ef-11f0-8695-0242ac170007',
                 '7c8eb1f8-44e2-4239-afaa-9cbf1fadf160')
GROUP BY 1,2,3 ORDER BY 1,4 DESC;


-- =====================================================================
-- J. STILL OUTSTANDING FROM site_visit: what type is resource_instances.name?
-- Needed by the resource-name resolver. Never actually run.
-- =====================================================================
SELECT pg_typeof(name) AS name_type, name, descriptors
FROM public.resource_instances LIMIT 3;