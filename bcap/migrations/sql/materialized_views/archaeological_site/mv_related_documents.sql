...
-- ---------------------------------------------------------------------
-- related_documents  (cardinality 1)  children: related_site_documents, site_images, publication_reference
-- ---------------------------------------------------------------------
...
site_images AS (
    SELECT t.parenttileid AS parenttileid,
           jsonb_agg(jsonb_build_object(
            ...
            'image_date', to_date(NULLIF(t.tiledata ->> 'c8162fa8-01d8-11f0-850c-0242ac170007', ''), 'YYYY-MM-DD'),
