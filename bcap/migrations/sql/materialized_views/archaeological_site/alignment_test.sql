...
WITH v AS (
  ...
  SELECT resourceinstanceid, external_url_count AS n, 'resource_flat_v1.external_url' AS grp,
         ARRAY[arches_util.nslots(external_url),
               arches_util.nslots(external_url_label),
               arches_util.nslots(external_url_type),
               arches_util.nslots(external_url_type_ids)] AS slots,
         ARRAY['external_url', 'external_url_label', 'external_url_type', 'external_url_type_ids']::text[] AS colnames
  ...
  SELECT resourceinstanceid, authority_count AS n, 'resource_flat_v1.authority' AS grp,
         ARRAY[arches_util.nslots(authority_protection_type),
               ...
               arches_util.nslots(authority_description),
               arches_util.nslots(authority_start_date),
               arches_util.nslots(authority_end_date)] AS slots,
         ARRAY['authority_protection_type', 'authority_protection_type_ids', 'legislative_act', 'legislative_act_ids', 'reference_number', 'authority_description', 'authority_start_date', 'authority_end_date']::text[] AS colnames
  ...
  SELECT resourceinstanceid, site_names_count AS n, 'resource_flat_v1.site_names' AS grp,
         ...
         ARRAY['name', 'name_type', 'name_type_ids', 'name_remarks', 'assigned_or_reported_by', 'assigned_or_reported_by_ids', 'assigned_or_reported_date']::text[] AS colnames
  ...
  SELECT resourceinstanceid, site_decision_count AS n, 'resource_flat_v1.site_decision' AS grp,
         ARRAY[arches_util.nslots(site_decision),
               arches_util.nslots(site_decision_ids),
               arches_util.nslots(decision_registration_status),
               ...
               arches_util.nslots(recommendation_date)] AS slots,
         ARRAY['site_decision', 'site_decision_ids', 'decision_registration_status', 'decision_registration_status_ids', 'decision_description', 'decision_made_by', 'decision_made_by_ids', 'decision_date', 'recommended_by', 'recommended_by_ids', 'recommendation_date']::text[] AS colnames
  ...
  SELECT resourceinstanceid, general_remark_information_count AS n, 'resource_flat_v1.general_remark_information' AS grp,
         ARRAY[arches_util.nslots(general_remark),
               arches_util.nslots(general_remark_source),
               arches_util.nslots(general_remark_source_ids),
               arches_util.nslots(general_remark_date)] AS slots,
         ARRAY['general_remark', 'general_remark_source', 'general_remark_source_ids', 'general_remark_date']::text[] AS colnames
  ...
  SELECT resourceinstanceid, restricted_information_count AS n, 'resource_flat_v1.restricted_information' AS grp,
         ARRAY[arches_util.nslots(restricted_remark),
               arches_util.nslots(restricted_person),
               arches_util.nslots(restricted_person_ids),
               arches_util.nslots(restricted_entry_date)] AS slots,
         ARRAY['restricted_remark', 'restricted_person', 'restricted_person_ids', 'restricted_entry_date']::text[] AS colnames
  ...
  SELECT resourceinstanceid, conviction_count AS n, 'resource_flat_v1.conviction' AS grp,
         ARRAY[arches_util.nslots(conviction_details),
               arches_util.nslots(conviction_date)] AS slots,
         ARRAY['conviction_details', 'conviction_date']::text[] AS colnames
  ...
  SELECT resourceinstanceid, biogeography_count AS n, 'site_location_flat_v1.biogeography' AS grp,
         ARRAY[arches_util.nslots(biogeography_type),
               ...
               arches_util.nslots(biogeography_name),
               arches_util.nslots(biogeography_description)] AS slots,
         ARRAY['biogeography_type', 'biogeography_type_ids', 'biogeography_name', 'biogeography_description']::text[] AS colnames
