...
import mapFilterUtils from 'bcgov_arches_common/bcgov_arches_common/media/js/utils/map-filter-utils';
const popupDataProvider = {
    ...
    getPopupTemplate: function (features) {
        if (
            _.some(features, function (feature) {
                return (
                    feature.sourceLayer ===
                    'WHSE_CADASTRE.PMBC_PARCEL_FABRIC_POLY_SVW'
                );
    ...
    sendFeatureToMapFilter: function (popupFeatureObject, addToFilter) {
        if (mapFilterUtils.isArchesGeometry(popupFeatureObject.feature)) {
            ...
            const feature = mapFilterUtils.getFeatureFromWFS(
