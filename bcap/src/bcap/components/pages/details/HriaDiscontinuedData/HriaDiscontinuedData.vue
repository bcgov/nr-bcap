...
type Cache = Record<string, HriaDiscontinuedDataSchema | null>;
const cache = ref<Cache>({});
const current = ref<HriaDiscontinuedDataSchema | null>(null);
...
watchEffect(async () => {
    ...
    if (!(resourceId in cache.value)) {
        getResourceData("hria_discontinued_data", resourceId).then((data) => {
            cache.value[resourceId] = data as HriaDiscontinuedDataSchema;
