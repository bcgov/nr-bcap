...
type Cache = Record<string, LegislativeActSchema | null>;
const cache = ref<Cache>({});
const current = ref<LegislativeActSchema | null>(null);
...
watchEffect(async () => {
    ...
    if (!(resourceId in cache.value)) {
        getResourceData("legislative_act", resourceId).then((data) => {
            cache.value[resourceId] = data as LegislativeActSchema;
