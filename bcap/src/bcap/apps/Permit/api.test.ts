...
vi.mock('arches', () => ({
    default: {
        urls: {
            api_resource_blank: (graphSlug: string) => `/mock/blank/${graphSlug}`,
            api_resource_draft: (graphSlug: string) => `/mock/draft/${graphSlug}`,
            api_resource_create: (graphSlug: string) => `/mock/create/${graphSlug}`,
        },
    },
}));
...
function mockFetchError(status: number, statusText: string, body: any = null) {
    return vi.fn().mockResolvedValue({
        ...
        status,
        statusText,
        json: body ? vi.fn().mockResolvedValue(body) : vi.fn().mockRejectedValue(new Error('JSON parse error')),
    });
}
...
describe('Permit API', () => {
    ...
    describe('getBlankPermitApplication', () => {
        it('fetches a blank permit application JSON', async () => {
            ...
            expect(fetch).toHaveBeenCalledWith('/mock/blank/permit_application?format=json', {});
    ...
    describe('fetchDrafts', () => {
        it('returns results array when paginated', async () => {
            vi.stubGlobal('fetch', mockFetchOk({ results: [{ id: 'draft-1' }] }));
    ...
    describe('fetchMyProjects', () => {
        it('returns results array when paginated', async () => {
            vi.stubGlobal('fetch', mockFetchOk({ results: [{ id: 'proj-1' }] }));
            const result = await fetchMyProjects();
            expect(fetch).toHaveBeenCalledWith('/bcap/api/dashboard/external?status=CREATED_BY_ME', expect.any(Object));
    ...
    describe('submitApplication', () => {
        it('POSTs the final resource and DELETEs the draft on success', async () => {
            ...
            // Assert POST request
            expect(fetchMock).toHaveBeenNthCalledWith(1, '/mock/create/permit_application', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': 'mock-csrf-token',
                },
                body: JSON.stringify({ draft_id: 'draft-123', aliased_data: payload }),
            });

            // Assert DELETE request
            expect(fetchMock).toHaveBeenNthCalledWith(2, '/mock/draft/permit_application/draft-123', {
                method: 'DELETE',
        ...
        it('throws an error with JSON details when POST fails', async () => {
            ...
            vi.stubGlobal('fetch', mockFetchError(400, 'Bad Request', errorDetails));

            await expect(submitApplication('draft-123', {})).rejects.toThrow(
                'Status 400: {"detail":"Validation failed"}'
            );
            expect(console.error).toHaveBeenCalledWith('Django 400 Error Details:', errorDetails);
        });
        ...
        it('throws a fallback error when POST fails and response is not JSON', async () => {
            ...
            await expect(submitApplication('draft-12