    ...
    def append_dsl(self, search_query_object, **kwargs):
        ...
        for term in JSONDeserializer().deserialize(querystring_params):
            if term["type"] == "term" or term["type"] == "string":
                ...
                if term["type"] == "term":
                    ...
                    try:
                        ...
                        if exact_term:
                            ...
                            string_filter.dsl["bool"]["minimum_should_match"] = 1
                ...
                if search_displayname:
                    displayname_query = Bool()
                    displayname_query.should(
                        Wildcard(
                    ...
                    displayname_query.dsl["bool"]["minimum_should_match"] = 1
                    nested_displayname_filter = Nested(path="displayname", query=displayname_query)

                    combined_filter = Bool()
                    combined_filter.should(nested_string_filter)
                    combined_filter.should(nested_displayname_filter)
                    combined_filter.dsl["bool"]["minimum_should_match"] = 1

                    if term["inverted"]:
                        search_query.must_not(combined_filter)
                    else:
                        search_query.must(combined_filter)
